...
export function calculateStageV3({
  uniqueWallets,
  gasUsage,
}: {
  uniqueWallets: number
  ...
  if (uniqueWallets >= 100_000 && gasUsage >= 500_000_000_000n) {
    ...
    return 4
  } else if (uniqueWallets >= 1_000 && gasUsage >= 5_000_000_000n) {
    return 3
  } else if (uniqueWallets >= 100 && gasUsage >= 500_000_000n) {
    return 2
  } else {
...
export function calculateRewards({
  ...
  const rewardsPerReferrer = Object.entries(referrerKpis).map(
    ([referrerId, kpi]) => {
      ...
      // Only qualified stages (1+) get quality user score bonuses. Stage 0 and 1 get 0 quality user score bonus.
      const qualityScore = qualityUserScores[referrerId] ?? 0
      const qualityUserScoreProportion =
        isExcluded || totalQualityUserScore === 0 || stage === 0 || stage === 1
          ? BigNumber(0)
