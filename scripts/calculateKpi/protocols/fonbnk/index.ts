...
/**
 * Retrieves all token transfer transactions from Fonbnk payout wallets to a specific user.
 *
...
/**
 * Calculates transaction volume generated for Fonbnk mobile telecom services.
 *
 * **KPI Unit**: USD (United States Dollars)
 *
 * **Business Purpose**: Measures the transaction volume attributable to a specific user's mobile telecom
 * service purchases through Fonbnk platform. This metric quantifies the economic activity generated by user
 * interactions with mobile airtime and data services and supports transaction volume analysis.
 *
 ...
 *
 * **Data Sources**:
 * - **HyperSync**: Token transfer events from user wallets to Fonbnk payout addresses
 ...
 * - Transaction volume is measured by token transfers to Fonbnk payout wallets
 * - User's volume contribution is the sum of all transfers within the time window
 * - USD conversion uses token prices at time of each transaction for accuracy
 * - Only transfers to verified Fonbnk payout addresses are included
 * - Transaction volume represents actual mobile service purchases
 *
 ...
 * 2. Queries token transfer events from user wallet to payout addresses across all supported networks
 ...
 * 6. Returns total transaction volume representing mobile service purchases
 *
 * @param params - Calculation parameters
 ...
 *
 * @returns Promise resolving to total transaction volume in USD for mobile telecom services
 */
