...
describe('getDailyMeanTvl', () => {
    it('should throw an error if endTimestamp is in the future', async () => {
        ...
        await expect(getDailyMeanTvl({ vaultInfo, address, startTimestamp, endTimestamp, nowTimestamp })).rejects.toThrow('Cannot have an endTimestamp in the future')
    })
    it('should return the correct daily mean TVL', async () => {
        ...
        const result = await getDailyMeanTvl({ vaultInfo, address, startTimestamp, endTimestamp, nowTimestamp })
