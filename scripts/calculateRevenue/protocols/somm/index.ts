...
export async function getDailyMeanTvl({
    ...
    for (const tvlEvent of tvlEvents) {
        let daysInRange = 0
        if(prevTimestamp.getTime() >= endTimestamp.getTime() && tvlEvent.timestamp.getTime() < endTimestamp.getTime()) {
            daysInRange = (endTimestamp.getTime() - tvlEvent.timestamp.getTime()) / ONE_DAY
        } else if(tvlEvent.timestamp.getTime() < endTimestamp.getTime()) {
            daysInRange = (prevTimestamp.getTime() - tvlEvent.timestamp.getTime()) / ONE_DAY
        }
        tvlDays += (daysInRange * currentTvl)
        currentTvl-= tvlEvent.amount
        prevTimestamp = tvlEvent.timestamp
    }
    tvlDays += (prevTimestamp.getTime() - startTimestamp.getTime()) / ONE_DAY * currentTvl
    return tvlDays / ((endTimestamp.getTime() - startTimestamp.getTime()) / ONE_DAY)
}
