 ...
 */
contract RewardPool is Initializable, AccessControlDefaultAdminRulesUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Events
    event PoolInitialized(address indexed poolToken, bytes32 rewardFunctionId, uint256 timelock);
    event Deposit(uint256 amount);
    event Withdraw(uint256 amount);
    event TimelockExtended(uint256 newTimelock);
    event AddReward(address indexed user, uint256 amount, bytes32 rewardsAllocationId);
     ...
     * @param _timelock Block number when manager withdrawals will be allowed
     */
    function initialize(
        address _poolToken,
     ...
     * @param blockNumber Future block number when withdrawals will be allowed
     * @notice Allowed only for address with MANAGER_ROLE
     */
    function extendTimelock(uint256 blockNumber) external onlyRole(MANAGER_ROLE) {
        _setTimelock(blockNumber);   
        emit TimelockExtended(blockNumber);
    }
    ...
    function withdraw(uint256 amount) external onlyRole(MANAGER_ROLE) nonReentrant {
        if (block.number < timelock) revert TimelockNotExpired(block.number, timelock);
     ...
     * @param rewardsAllocationId Bytes32 identifier of the rewards allocation (e.g. git commit hash)
     * @notice Allowed only for address with MANAGER_ROLE
     */
    function addRewards(
        address[] calldata users, 
        uint256[] calldata amounts, 
        bytes32 rewardsAllocationId
    ) external onlyRole(MANAGER_ROLE) {
        uint256 usersLength = users.length;
        uint256 amountsLength = amounts.length;
        if (usersLength != amountsLength) revert ArraysLengthMismatch(usersLength, amountsLength);
        ...
        for (uint256 i = 0; i < usersLength;) {
            ...
            emit AddReward(users[i], amounts[i], rewardsAllocationId);
    ...
    function claimReward(uint256 amount) external nonReentrant {
        ...
        if (amount > userPendingRewards) revert InsufficientRewardBalance(amount, userPendingRewards);
     ...
     * @param blockNumber Block number when withdrawals will be allowed
     */
    function _setTimelock(uint256 blockNumber) internal {
        if (blockNumber <= block.number) revert TimelockMustBeInTheFuture();
     ...
     * @notice Allowed only for address with MANAGER_ROLE
     */
    function rescueToken(address rescuedToken) external onlyRole(MANAGER_ROLE) nonReentrant {
