  ...
  ERC2771ContextUpgradeable
{
   ...
   *      1. On-chain: offchainVerificationHash = bytes32(0), uses txHash + chainId for verification
   *      2. Off-chain: offchainVerificationHash != bytes32(0), txHash + chainId ignored in events
   */
  struct ReferralDataV2 {
    address user;
    address rewardsProvider;
    address rewardsConsumer;
    bytes32 txHash; // Used for on-chain referrals, on-chain only
    string chainId; // Used for on-chain referrals, on-chain only
    bytes32 offchainVerificationHash; // Hash of verification data checked off-chain by trusted registrar, off-chain only
  }
   ...
   * @dev Requires REFERRAL_REGISTRAR_ROLE. Signature verification is performed off-chain by trusted registrar
   * @param referrals Array of referral data to register
   */
  function batchRegisterReferral(
    ...
    for (uint256 i = 0; i < referrals.length; i++) {
      ...
      // Determine if this is an off-chain referral
      bool isOffchainReferral = referral.offchainVerificationHash != bytes32(0);

      // Set event values based on referral type
      string memory eventChainId = isOffchainReferral ? '' : referral.chainId;
      bytes32 eventTxHash = isOffchainReferral ? bytes32(0) : referral.txHash;

      // Process the referral (no on-chain signature verification)
      ReferralStatus status = _registerReferral(
        referral.user,
        referral.rewardsProvider,
        referral.rewardsConsumer
   ...
   * @dev Internal function that returns status instead of emitting events. Signature verification handled off-chain.
